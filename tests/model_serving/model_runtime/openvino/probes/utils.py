"""Utilities for OVMS readiness and liveness probe validation."""
...
def get_kserve_container(pod: Pod) -> Any:
    ...
    Args:
        pod: Predictor pod for the OVMS InferenceService.
...
def resolve_http_get(probe: dict[str, Any] | None, *, default_port: int = OPENVINO_REST_PORT) -> dict[str, Any]:
    ...
    When httpGet is absent, fall back to the standard OVMS REST port and /v2/health/ready path.
    """
    if probe:
...
def exec_http_probe(pod: Pod, http_get: dict[str, Any]) -> str:
    ...
    if scheme == "HTTPS":
        curl_cmd.append("-k")  # Skip cert validation for pod-internal HTTPS probes (current probes use HTTP)
    curl_cmd.append(url)
...
def exec_ovms_health_check(pod: Pod, http_get: dict[str, Any]) -> str:
    """Execute health check probe on OVMS pod using the configured probe path."""
